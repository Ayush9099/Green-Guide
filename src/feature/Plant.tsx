...
export default function PlantManagement() {
  ...
  const handleTreflePlantSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedPlant = trefleData.find(plant => plant.id.toString() === selectedPlantId);
    ...
    if (selectedPlant) {
      setFormData(prev => ({
        ...
        generalInfo: {
          ...
          description: `${selectedPlant.family_common_name ? `Family: ${selectedPlant.family_common_name}. ` : ""}${selectedPlant.bibliography}`,
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      ...
      {showAddPlant && (
        ...
        >
          ...
          <input
            ...
            className="w-full p-2 mb-2 border rounded"
          />
          {formData.generalInfo.img && typeof formData.generalInfo.img === 'string' && (
            <img src={formData.generalInfo.img} alt="Selected plant" className="w-full h-auto mb-2 rounded" />
          )}
          ...
          plantsData.map((plant) => (
            <div key={plant._id} className="border border-gray-300 rounded-lg p-4 mb-4 shadow-md hover:shadow-lg transition-shadow duration-200">
              <h3 className="text-xl font-semibold mb-2">
                <Link to={`/plants/${plant._id}`} className="text-green-600 hover:underline">
                  {plant.generalInfo?.plantName || "Unnamed Plant"}
                </Link>
              </h3>
              <p className="text-gray-700 mb-2">{plant.generalInfo?.description || "No description available"}</p>
              {plant.generalInfo?.img && (
                <div className="flex justify-center mb-4">
                  <Link to={`/plants/${plant._id}`}>
                    <img src={plant.generalInfo.img} alt={plant.generalInfo?.plantName || "Plant image"} className="w-full h-auto rounded-md cursor-pointer" />
                  </Link>
                </div>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {trefleData.length > 0 ? (
          trefleData.map((plant) => (
            <div key={plant.id} className="border border-gray-300 rounded-lg p-4 mb-4 shadow-md hover:shadow-lg transition-shadow duration-200">
              <h3 className="text-xl font-semibold mb-2">
                {plant.common_name || plant.scientific_name}
              </h3>
              <p className="text-gray-700 mb-2">Scientific Name: {plant.scientific_name}</p>
              ...
              <p className="text-gray-700 mb-2">Bibliography: {plant.bibliography}</p>
              ...
              {plant.family_common_name && (
                <p className="text-gray-700 mb-2">Family Common Name: {plant.family_common_name}</p>
              )}
              {plant.image_url && (
                <div className="flex justify-center mb-4">
                  <img src={plant.image_url} alt={plant.common_name || plant.scientific_name} className="w-full h-auto rounded-md" />
                </div>
              )}
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside">
                    {plant.synonyms.map((synonym, index) => (
                      <li key={index} className="text-sm text-gray-600">{synonym}</li>
                    ))}
                  </ul>
                </div>
              )}
          ...
          <p className="text-center text-gray-600">No Trefle.io plants found.</p>
        )}
      </div>
    </div>
  );
